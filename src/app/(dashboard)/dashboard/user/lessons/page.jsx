  ...
  const handleVisibilityChange = async (lessonId, newVisibility) => {
    try {
      ...
      if (response.ok) {
        ...
        setLessons((prev) =>
          prev.map((l) => (l._id === lessonId ? { ...l, visibility: newVisibility } : l))
        );
      } else {
  ...
  const handleAccessLevelChange = async (lessonId, newAccessLevel) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setLessons((prev) =>
          prev.map((l) => (l._id === lessonId ? { ...l, accessLevel: newAccessLevel } : l))
        );
      } else {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-[15px] text-zinc-600 dark:text-zinc-400 mt-1">
            Manage, configure access settings, and track performance of your contributions.
          </p>
        </div>
        
        <Link href="/dashboard/user/add-lesson">
          <Button className="bg-[#0f766e] hover:bg-[#0d6e63] text-white font-semibold px-5 py-2.5 rounded-xl transition-colors shadow-sm flex items-center gap-2 cursor-pointer">
            ...
            <tbody className="divide-y divide-zinc-200 dark:divide-zinc-800 text-[14px]">
              {isLoading ? (
                ...
                <tr>
                  <td colSpan="7" className="py-16 text-center text-zinc-500">
                    <div className="flex flex-col items-center justify-center gap-2">
                      ...
                      <p className="font-semibold text-zinc-700 dark:text-zinc-300">No lessons created yet</p>
                      <p className="text-[13px] text-zinc-400">
                        {searchQuery ? "No lessons match your search keyword." : "Draft your first module to share knowledge."}
                      </p>
                    </div>
                  </td>
                </tr>
                ...
                currentLessons.map((lesson) => {
                  return (
                    <tr key={lesson._id} className="hover:bg-zinc-50/50 dark:hover:bg-zinc-800/40 transition-colors">
                          ...
                          <div className="inline-block">
                            <select
                              ...
                              className={`bg-[#f9fafb] dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-lg px-2.5 py-1.5 text-[13px] font-semibold outline-none transition-colors ${
                                !isPremiumUser ? 'cursor-not-allowed opacity-60 text-zinc-500' : 'cursor-pointer text-zinc-800 dark:text-zinc-200 focus:border-[#0f766e]'
                              }`}
                            >
