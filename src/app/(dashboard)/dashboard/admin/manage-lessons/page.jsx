...
export default function AdminManageLessonsPage() {
  ...
  const handleToggleFeatured = async (lessonId, currentFeatured) => {
    ...
    try {
      ...
      if (response.ok) {
        toast.success(
          newFeatured ? "Lesson marked as Featured!" : "Removed from Featured.",
        );
        setLessons(
          lessons.map((l) =>
  ...
  const handleToggleReviewed = async (lessonId, currentReviewed) => {
    ...
    try {
      ...
      if (response.ok) {
        toast.success(
          newReviewed ? "Marked as Reviewed." : "Marked as Pending Review.",
        );
        setLessons(
          lessons.map((l) =>
            l._id === lessonId ? { ...l, isReviewed: newReviewed } : l,
          ),
        );
      } else {
  ...
  // Stats Calculations
  const publicLessonsCount = lessons.filter(
    (l) => l.visibility === "Public",
  ).length;
  const privateLessonsCount = lessons.filter(
    (l) => l.visibility === "Private" || l.visibility === "Draft",
  ).length;
  const flaggedLessonsCount = lessons.filter((l) =>
    openReportedLessonIds.has(String(l._id)),
  ).length;
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-[15px] text-zinc-600 dark:text-zinc-400 mt-1">
            Oversee all community modules, moderate flagged content, and curate
            featured wisdom.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-white dark:bg-zinc-900 border border-red-200 dark:border-red-900/40 rounded-2xl p-6 flex flex-col justify-between shadow-sm">
          ...
          <div>
            ...
            <p className="text-[12px] text-red-500/80 dark:text-red-400/80 mt-1">
              Requires moderation
            </p>
          </div>
        </div>
                ...
                return (
                  ...
                  >
                    ...
                    <td className="py-4 px-6 whitespace-nowrap">
                      ...
                      >
                        <span
                          className={`w-1.5 h-1.5 rounded-full ${lesson.visibility === "Public" ? "bg-indigo-500" : "bg-zinc-400"}`}
                        ></span>
                        {lesson.visibility || "Public"}
                      </span>
                    </td>
                    ...
                    <td className="py-4 px-6 text-right whitespace-nowrap">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        >
                          <Star
                            className={`w-4 h-4 ${lesson.isFeatured ? "fill-amber-500 text-amber-500" : ""}`}
                          />
                        </button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-xs p-4 animate-in fade-in duration-200">
          ...
          >
            ...
            <p className="text-[13px] text-zinc-500 dark:text-zinc-400 leading-relaxed mb-6">
              ...
              ? All reports associated with this module will be automatically
              resolved.
            </p>
